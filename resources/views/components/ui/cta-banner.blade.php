...
<section class="py-20 bg-gray-100 relative overflow-hidden border-t-[8px] theme-border content-auto">
    ...
    <div class="max-w-5xl mx-auto text-center px-4 relative z-10">
        ...
        <div class="flex flex-col sm:flex-row justify-center gap-6">
            <a href="{{ $buttonLink }}" class="bg-black text-white px-8 py-4 min-h-[56px] font-extrabold text-sm uppercase tracking-widest hover:theme-bg border-4 border-transparent transition-surface shadow-[8px_8px_0_#0033a0] hover:-translate-y-1 hover:shadow-[12px_12px_0_#0033a0]">
                {{ $buttonText }}
            </a>
            <a href="{{ preg_replace('/\D+/', '', (string) ($siteSettings->whatsapp_number ?? '')) ? 'https://wa.me/' . preg_replace('/\D+/', '', (string) ($siteSettings->whatsapp_number ?? '')) : route('contact.index') }}" class="bg-transparent text-black border-4 border-black px-8 py-4 min-h-[56px] font-extrabold text-sm uppercase tracking-widest hover:bg-black hover:text-white transition-colors flex items-center justify-center gap-3">
